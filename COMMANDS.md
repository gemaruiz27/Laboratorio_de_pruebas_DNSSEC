...
> **Nota:** esta sección es una reconstrucción a partir de los archivos encontrados en el segundo backup (`Ksigned.+013+...`, `dsset-*`, `db.*.signed`), no es el historial exacto de comandos. El algoritmo `013` corresponde a ECDSAP256SHA256. Cada zona tiene dos pares de llaves, consistente con el esquema KSK/ZSK. Ajusta parámetros si recuerdas otros distintos.
...
`dns-lab/gamma/zones/db.gamma.signed` no tiene llaves ni archivo `dsset-`, a pesar del nombre. Probablemente sea un caso de prueba deliberado (zona delegada sin firmar, para probar validación fallida), no una zona realmente firmada.
```bash
